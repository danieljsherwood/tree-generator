function leafstl(vertices)
% Set up matrix for points of leaf
points=[]
% For each vertex, add the point onto the matrix
for n=1:vertices
    angle=n/vertices*360
    x=sind(angle)
    y=cosd(angle)
    points(end+1,:)=[x,y]
end
% Add the first point again as the last point
points(end+1,:)=points(1,:)
% Make delaunay triangulation of the points
DT =delaunay(points(:,1),points(:,2))
% Triangulate points
TR=triangulation(DT,points(:,1),points(:,2))
% Write stl file
stlwrite(TR,'Leaf.stl','binary')
